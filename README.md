...
**🔗 상세 포트폴리오:** [juseong.dev](https://www.juseong.dev)
...
| | |
| :--- | :--- |
| **프로젝트 목표** | 여러 곳에 흩어진 여행 관련 데이터를 통합 분석하여, 사용자에게 '여행지 매력도 점수'를 제공하는 End-to-End 데이터 플랫폼 구축 |
| **핵심 역할** | 프로젝트 오너(Owner)로서 초기 아이디어부터 최종 아키텍처 설계 및 핵심 ETL 로직 구현까지 전 과정을 총괄 |
| **핵심 성과** | **1. 안정적인 데이터 수집:** 429 Error를 원천 해결하여 불안정한 외부 API 환경에서 데이터 유실 없는 수집 기반 구축<br>**2. 분석 커버리지 100% 달성:** 키 값 불일치로 유실되던 90%의 데이터를 복원하여 분석 가능한 데이터 범위 완성<br>**3. 고성능 하이브리드 아키텍처 설계:** 실시간 처리의 한계를 진단하고, 배치 파이프라인을 도입하여 안정성과 확장성 확보 |
| **기술 스택** | Azure Databricks, Azure Functions, Spark, Python, SQL, Unity Catalog, Delta Lake, Power BI |
